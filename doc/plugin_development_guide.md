...
エディタが重いパース処理や複雑な UI 構築を行う場合、本体や外部（ナビゲーション等）から「特定の項目を選択した状態で開いてほしい」といった要求（パラメータ）が来ることがあります。

確実にパラメータを適用するために、エディタは以下のルールに従う必要があります。

1.  **`set_params(params)` を実装する**: 外部から送られたパラメータ（例：ジャンプ先のID）を処理するメソッドを用意します。
2.  **`notify_editor_ready(widget)` を呼ぶ**: 初期化が完全に終わったタイミングで本体に通知します。

本体は、通知を受けるまでパラメータの送信を待機し、準備ができ次第 `set_params` を呼び出します。これにより、タイミングの不整合による「項目の選択漏れ」を防ぐことができます。
